...
2. The frontend creates a remittance workspace and asks the backend for a Stellar-ready action quote.
...
- Soroban Rust smart contract for deterministic remittance workflow state.
...
- `record_remittance_record(id, actor, amount, score)`
...
```bash
stellar contract deploy --wasm target/wasm32v1-none/release/ofw_insurance_premium_collector_contract.wasm --source alice --network testnet -- --admin alice --asset CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC --project_name "OFW Insurance Premium Collector"
```
...
This project is generated as an original StellarX Philippines full-stack starter from idea #21. It uses Stellar testnet, Soroban, Freighter, and current ecosystem SDKs as an extensible hackathon baseline rather than a barely modified template.
